...
class FifoSimulator:
    ...
    def fifo(self, tick):
        ...
        d = {input_pkts: [] for input_pkts in range (self.num_ports)}

        for input_queue in self.input_queues:
            if (len(input_queue) > 0):
                d[input_queue[0].output_port].append(input_queue[0])

        for output_port in range(self.num_ports):
            if (len(d[output_port]) > 0):
                choice_pkt = random.choice (d[output_port])
                self.input_queues[choice_pkt.input_port] = self.input_queues[choice_pkt.input_port][1:]

                delay = tick - choice_pkt.arrival_tick
